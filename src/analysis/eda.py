...
class ExploratoryDataAnalysis:
    ...
    def _analyze_data_overview(self, df: pd.DataFrame) -> Dict:
        ...
        for col in df.select_dtypes(include=[np.number]).columns:
            if df[col].std() < 0.01:
                overview['zero_variance_features'].append(col)

        for col in df.select_dtypes(include=['object']).columns:
            if df[col].nunique() > len(df) * 0.5:
                overview['high_cardinality_features'].append(col)
    ...
    def _analyze_target(self, df: pd.DataFrame, target_col: str) -> Dict:
        ...
        target_analysis = {
            ...
            'percentage': (df[target_col].value_counts(normalize=True) * 100).round(2).to_dict(),
            'class_ratio': None,
            'entropy': None
        }
        ...
        if df[target_col].nunique() == 2:
            ...
            target_analysis['class_ratio'] = round(counts.iloc[0] / counts.iloc[1], 2)

            # Calculate entropy
            probs = df[target_col].value_counts(normalize=True)
            target_analysis['entropy'] = -sum(p * np.log2(p) for p in probs if p > 0)
    ...
    def _analyze_correlations(self, df: pd.DataFrame) -> Dict:
        ...
        if 'churned' in numeric_df.columns:
            ...
            target_correlations = target_corr[1:21].to_dict()  # Top 20 excluding target itself
    ...
    def _analyze_segments(self, df: pd.DataFrame, target_col: str) -> Dict:
        ...
        if 'customer_segment' in df.columns and target_col in df.columns:
            ...
            segments['by_customer_segment'] = segment_analysis.to_dict()
        ...
        if 'age_group' in df.columns and target_col in df.columns:
            ...
            segments['by_age_group'] = age_analysis.to_dict()
        ...
        if 'rfm_segment' in df.columns and target_col in df.columns:
            ...
            segments['by_rfm_segment'] = rfm_analysis.to_dict()
        ...
        if 'value_segment' in df.columns and target_col in df.columns:
            ...
            segments['by_value_segment'] = value_analysis.to_dict()
